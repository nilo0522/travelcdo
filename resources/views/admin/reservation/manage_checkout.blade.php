                ...
                <div class="row mb-2">
                  ...
                  <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                      ...
                      <li class="breadcrumb-item active">Reservation/<a href="{{ url('admin/reservation-manage-confirmed') }}">{{ session('status') }}</a></li>
                    </ol>
                  </div>
                </div>
              </div><!-- /.container-fluid -->
                ...
                @endphp
                   @foreach($res_all as  $res) 
                    ...
                    @foreach($customer as $c)
                      @if($res->room_id==$r->id && $res->cus_id == $c->id && $res->book_status==session('status') )
                 ...
                 <tr>
                   ...
                   <td>₱ {{ number_format($res->total) }}</td>
                    ...
                    <span class="badge badge-warning">{{ $res->book_status }}</span> 
                      @endif
                      @if(session('status')=="Confirmed")
                    <span class="badge badge-success">{{ $res->book_status }}</span> 
                      @endif
                      @if(session('status')=="Checkin")
                    <span class="badge badge-primary">{{ $res->book_status }}</span> 
                      @endif

                   
                     </td>
                     <td>
                       <form method="post" action="{{ action('CheckoutController@destroy',$res->id) }}">
                        @method('delete')
                           ...
                           @if(session('status')=="Pending")
                            ...
                            <button type="submit" class="btn btn-sm btn-dark">
                              <i class="fa fa-calendar-times"> Checkout</i>
                            </button>
                            @endif
                            @csrf
                          
                            <input type="hidden" name="h_id" value="{{ $res->h_id }}">
                            <input type="hidden" name="qty" value="{{ $res->qty }}">
                            <input type="hidden" name="night" value="{{ $res->night }}">
                            <input type="hidden" name="total" value="{{ $res->total }}">
                            <input type="hidden" name="c_type" value="{{ $c->c_type }}">
                            <input type="hidden" name="book_date" value="{{ $res->created_at }}">
                          ...
                          <div class="col-md-12">
                            <div class="card">
                              <div class="card-body">
                               ...
                               <div  style="display: table;" class="table table-responsive table-bordered ">
