    ...
    <section class="content ">
      <div class="card">
        <div class="card-header">
              <h3 class="card-title"><b>LIST OF CHECKIN RESERVATION</b></h3>
                ...
                <tbody>
                 ...
                 @endphp
                   @foreach($res_all as $res) 
                    ...
                    @foreach($customer as $c)
                      @if($res->room_id==$r->id && $res->cus_id == $c->id && $res->book_status=="Checkin")
                    <form method="post" action="{{ action('CheckoutController@store') }}">
                        @csrf
                 ...
                 <tr>
                   ...
                   @foreach($res_all as $all)
                    @if($all->created_at == $res->created_at && $all->book_status=="Checkin") 
                       @php 
                       $qty+=$all->qty;
                       $total+=$all->total;
                   ...
                   <td>@if($res->book_status=="Checkin") <span class="badge badge-primary">{{ $res->book_status }}</span>  @endif
                   
                     </td><input type="hidden" name="book_date" value="{{ $res->created_at }}">

                   <td align="center">
                    <div class="btn-group">
                      <button type="submit"  class="btn btn-xs btn-dark"><i class="fa fa-calendar-times "> Checkout</i> </button><a href="{{ url('admin/reservation-manage-checkin/'.$c->id) }}" class="btn btn-xs btn-info"><i class="fa fa-tasks"></i> Manage</a>
                    </div></td>
                 </tr>
                              ...
                              <div class="card">
                               ...
                               <div class="card-body text-left">
                                <div class="col-md-12 row text-sm">
                                 ...
                                 @foreach($rooms as $r)
                                 @foreach($alls as $all)
                                  @if($all->room_id==$r->id && $all->cus_id == $res->cus_id && $all->book_status=="Checkin" ) 
                                       <div class="card-body">
                                   <div class="row">
                                     ...
                                     <div class="col-md-8 " style="margin-top: 10px">
                                       <a href="" data-toggle="modal" data-target="#room{{ $r->id }}"><label style="cursor: pointer"><span class="text-secondary">{{ $all->qty }}x</span> <span class=" text-success">
                                        ...
                                        @if($all->room_id==$room->id && $all->book_status=="Checkin")
                                        {{ $room->room_type }}
                                        @endif
                                        @endforeach
                                      </span></label></a>
